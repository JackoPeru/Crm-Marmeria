...
const {
  authenticateToken, requirePermission, requireRole, generateToken,
  hashPassword, verifyPassword, findUserByCredentials, readUsers,
  writeUsers, configureAuth, verifyToken,
...
const normalize = (type, raw = {}) => {
  ...
  if (type === 'material') {
    ...
    Object.assign(data, {
      unitPrice, price: unitPrice, stockQuantity, quantity: stockQuantity,
      stock: stockQuantity, minStockLevel, minQuantity: minStockLevel,
    });
  }
  if (['order', 'project', 'quote', 'invoice'].includes(type)) {
    ...
    if (data.amount != null || data.total != null) data.amount = Number(data.amount ?? data.total) || 0;
  }
  return data;
};
...
const hasEntityPermission = (user, type, action) => {
  ...
  return Boolean(entry && user?.permissions?.includes(`${entry[1].permission}.${action}`));
};
...
async function createCrmServer(options = {}) {
  ...
  app.get('/api/health', (req, res) => res.json({
    status: 'ok', version: '2.0.0', mode: 'central-server',
    hostname: options.serverName || 'crm-marmeria', serverId: options.serverId || null,
    port, timestamp: new Date().toISOString(), websocket: true,
  }));
  ...
  app.put('/api/auth/profile', authenticateToken, (req, res) => {
    try {
      ...
      if (users.some((user, i) => i !== index && ((updates.username && user.username === updates.username) || (updates.email && user.email === updates.email)))) {
  ...
  app.post('/api/users', authenticateToken, requireRole('admin'), async (req, res) => {
    try {
      ...
      const user = {
        id: crypto.randomUUID(), username, email, password: await hashPassword(password),
        firstName, lastName, role, permissions: Array.isArray(permissions) ? permissions : [],
  ...
  app.put('/api/users/:id', authenticateToken, requireRole('admin'), async (req, res) => {
    try {
      ...
      users[index] = {
        ...
        password: req.body.password ? await hashPassword(req.body.password) : users[index].password,
  ...
  // Route specifiche prima delle route /:id.
  ...
  app.get('/api/clients/stats', authenticateToken, requirePermission('clients.view'), (req, res) => {
    ...
    res.json({
      ...
      recentlyAdded: items.filter((item) => new Date(item.createdAt).getTime() > Date.now() - 604800000).length,
    });
  });
  ...
  app.get('/api/materials/stats', authenticateToken, requirePermission('materials.view'), (req, res) => {
    ...
    res.json({
      total: items.length, byCategory, lowStock: low.length, lowStockItems: low.length,
      totalValue: items.reduce((sum, item) => sum + Number(item.stockQuantity || 0) * Number(item.unitPrice || 0), 0),
    });
  });
  ...
  const updateOrderStatus = (req, res) => {
    try {
      const result = db.update(
        ...
        expectedVersionFrom(req), req.user, operationIdFrom(req),
      );
  ...
  app.get('/api/analytics/dashboard', authenticateToken, requirePermission('dashboard.view'), (req, res) => {
    ...
    res.json({
      ...
      totalRevenue: invoices.reduce((sum, item) => sum + Number(item.total || item.amount || 0), 0),
      ...
      lowStockMaterials: materials.filter((item) => Number(item.stockQuantity || 0) < Number(item.minStockLevel || 0)).length,
      recentClients: clients.filter((item) => new Date(item.createdAt).getTime() > Date.now() - 604800000).length,
    });
  });
  app.get('/api/analytics/daily/:date?', authenticateToken, requirePermission('dashboard.view'), (req, res) => {
    ...
    res.json({
      ...
      totalRevenue: items.reduce((sum, item) => sum + Number(item.amount || item.total || 0), 0),
      newClients: db.list('client').filter((item) => String(item.createdAt || '').slice(0, 10) === date).length,
      pendingOrders: items.filter((item) => ['pending', 'In Attesa'].includes(item.status)).length,
      completedOrders: items.filter((item) => ['completed', 'Completato'].includes(item.status)).length,
    });
  });
  app.get('/api/analytics/trends', authenticateToken, requirePermission('dashboard.view'), (req, res) => {
    ...
    for (const current = new Date(start); current <= end; current.setDate(current.getDate() + 1)) {
      ...
      const value = metric === 'clients'
        ...
        : metric === 'revenue'
          ? invoices.filter((item) => String(item.createdAt || '').slice(0, 10) === date).reduce((sum, item) => sum + Number(item.total || item.amount || 0), 0)
  ...
  app.get('/api/audit/:type/:id', authenticateToken, (req, res) => {
    if (!hasEntityPermission(req.user, req.params.type, 'view')) return res.status(403).json({ error: 'Permessi insufficienti' });
    res.json(db.listAudit({ type: req.params.type, id: req.params.id, limit: req.query.limit }));
  });

  app.get('/api/entity-attachments/:type/:id', authenticateToken, (req, res) => {
    try {
      if (!hasEntityPermission(req.user, req.params.type, 'view')) return res.status(403).json({ error: 'Permessi insufficienti' });
  ...
  app.post('/api/entity-attachments/:type/:id', authenticateToken, (req, res, next) => {
    if (!hasEntityPermission(req.user, req.params.type, 'edit')) return res.status(403).json({ error: 'Permessi insufficienti' });
    next();
    ...
    try {
      ...
      const items = (req.files || []).map((file) => db.addAttachment({
  ...
  app.get('/api/attachments/file/:id', authenticateToken, (req, res) => {
    ...
    if (!hasEntityPermission(req.user, attachment.entityType, 'view')) return res.status(403).json({ error: 'Permessi insufficienti' });
  ...
  app.delete('/api/attachments/file/:id', authenticateToken, (req, res) => {
    ...
    if (!hasEntityPermission(req.user, attachment.entityType, 'edit')) return res.status(403).json({ error: 'Permessi insufficienti' });
  ...
  app.post('/api/backup/clear', authenticateToken, requirePermission('settings.edit'), async (req, res) => {
    try {
      ...
      db.restoreJson({ data: Object.fromEntries(ENTITY_TYPES.map((type) => [type, []])) }, req.user);
      realtime.broadcast({ event: 'database.restored' });
  ...
  app.post('/api/backups/:name/restore', authenticateToken, requirePermission('settings.edit'), async (req, res) => {
    try {
      await db.createSnapshot('pre-ripristino');
      const snapshot = db.restoreSnapshot(req.params.name);
  ...
  const ensureDailyBackup = async () => {
    try {
      ...
      const alreadyCreated = db.listSnapshots().some((item) => item.createdAt.startsWith(today) && item.label === 'automatico');
  ...
  return {
    app, server, db, port, host,
    close: async () => {
      clearInterval(backupTimer);
      for (const client of realtime.wss.clients) client.close();
