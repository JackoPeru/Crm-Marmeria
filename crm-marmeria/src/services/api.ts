...
interface ReplayConfig extends AxiosRequestConfig { _replay?: boolean }
...
class ApiClient {
  ...
  constructor() {
    this.axiosInstance = axios.create({ baseURL: this.getBaseURL(), timeout: 12000, headers: { 'Content-Type': 'application/json' } });
    this.setupInterceptors();
  }
  getBaseURL(): string {
    return localStorage.getItem('crm_api_base_url') || import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:3001/api';
  }
  ...
  private setupInterceptors(): void {
    this.axiosInstance.interceptors.request.use((config: any) => {
      ...
      if (MUTATING.has(method)) {
        ...
        if (method === 'post' && ENTITY_CREATE.test(String(config.url || '')) && config.data && !(config.data instanceof FormData)) {
    ...
    this.axiosInstance.interceptors.response.use((response) => response, async (error) => {
      ...
      if (status === 409) window.dispatchEvent(new CustomEvent('crm-version-conflict', { detail: { url: config.url, current: error.response?.data?.current, message: error.response?.data?.error } }));
      const isNetworkFailure = error.code === 'ERR_NETWORK' || error.code === 'ECONNABORTED' || !error.response;
      const canQueue = isNetworkFailure && MUTATING.has(method) && !config._replay && !(config.data instanceof FormData) && !String(config.url || '').startsWith('/auth/');
      if (canQueue) {
        let data = config.data;
        if (typeof data === 'string') { try { data = JSON.parse(data); } catch {} }
        const id = String(config.headers?.['X-Operation-Id'] || operationId());
        await offlineQueue.add({ id, method: method as 'post' | 'put' | 'patch' | 'delete', url: String(config.url), data, headers: { 'X-Operation-Id': id, ...(config.headers?.['If-Match'] ? { 'If-Match': String(config.headers['If-Match']) } : {}) } });
        toast('Modifica salvata in coda: verrà inviata quando il server torna disponibile.', { id: 'offline-queued' });
        const optimistic = { ...(typeof data === 'object' && data ? data : {}), id: (data as any)?.id || String(config.url || '').split('/').filter(Boolean).pop(), _queued: true };
        return { data: optimistic, status: 202, statusText: 'Queued Offline', headers: {}, config } as AxiosResponse;
      }
      if (isNetworkFailure) {
        const previous = Number(localStorage.getItem('lastNetworkErrorToast') || 0);
        if (Date.now() - previous > 30000) {
          toast.error('Server centrale non raggiungibile. I dati disponibili restano consultabili.', { id: 'network-error' });
  ...
  async checkHealth(): Promise<boolean> {
    try { await this.axiosInstance.get('/health', { timeout: 5000 }); await this.replayOfflineQueue(); return true; }
    catch { return false; }
  }
  async replayOfflineQueue(): Promise<void> {
    ...
    try {
      for (const request of await offlineQueue.list()) {
        try {
          await this.axiosInstance.request({ method: request.method, url: request.url, data: request.data, headers: request.headers, _replay: true } as ReplayConfig);
          await offlineQueue.remove(request.id);
        } catch (error: any) {
          if ([400, 403, 404, 409].includes(error.response?.status)) {
            await offlineQueue.markFailure(request.id, error.response?.data?.error || error.message);
            window.dispatchEvent(new CustomEvent('crm-offline-operation-failed', { detail: { request, error: error.response?.data } }));
            continue;
          }
          await offlineQueue.markFailure(request.id, error.message || 'Server non raggiungibile');
          break;
        }
      }
