    ...
    <>
      ...
      <div className="mb-10 p-6 bg-white dark:bg-dark-card rounded-lg shadow-md">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          ...
          <button onClick={() => void loadBackups()} disabled={busy} className="p-2 border rounded-md" title="Aggiorna elenco">
            <RefreshCw size={18} />
          </button>
        </div>
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-5">
          Il server crea automaticamente un backup completo al giorno e conserva gli ultimi 30. Ogni backup include database SQLite e allegati.
        </p>
        <div className="flex flex-wrap gap-3 mb-6">
          <button onClick={createSnapshot} disabled={busy} className="px-4 py-2 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white rounded-md flex items-center gap-2">
            <Database size={18} /> Crea backup ora
          </button>
          <button onClick={exportJson} disabled={busy} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-md flex items-center gap-2">
          ...
          <label className={`px-4 py-2 rounded-md text-white flex items-center gap-2 ${busy ? 'bg-gray-400' : 'bg-indigo-600 hover:bg-indigo-700 cursor-pointer'}`}>
            ...
            <input type="file" accept="application/json,.json" onChange={importJson} disabled={busy} className="hidden" />
          </label>
        </div>
        {backups.length > 0 ? (
          <div className="space-y-2 max-h-72 overflow-y-auto">
            {backups.map((backup) => (
              <div key={backup.name} className="p-3 border rounded-md flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-medium">{backup.label === 'automatico' ? 'Backup automatico' : 'Backup manuale'}</p>
                  <p className="text-sm text-gray-500">{new Date(backup.createdAt).toLocaleString('it-IT')} · {formatBytes(backup.sizeBytes)}</p>
                </div>
                <button onClick={() => restoreSnapshot(backup)} disabled={busy} className="px-3 py-2 border rounded-md flex items-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          <p className="text-sm text-gray-500">Nessun backup elencato oppure account senza permesso per visualizzarli.</p>
        )}
        {status && (
