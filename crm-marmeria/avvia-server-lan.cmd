...
:port_status
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$connection = Get-NetTCPConnection -LocalPort %PORT% -State Listen -ErrorAction SilentlyContinue | Select-Object -First 1; if (-not $connection) { exit 1 }; $process = Get-CimInstance Win32_Process -Filter ('ProcessId = ' + $connection.OwningProcess) -ErrorAction SilentlyContinue; if (-not $process -or $process.Name -ne 'node.exe') { exit 2 }; if ($process.CommandLine -match 'index\.js') { exit 0 }; exit 3"
if %ERRORLEVEL% EQU 3 (
  curl.exe -k -sS --max-time 3 "https://127.0.0.1:%PORT%/api/health" 2>nul | findstr /C:"\"mode\":\"central-server\"" >nul
