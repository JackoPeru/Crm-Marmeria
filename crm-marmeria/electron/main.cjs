...
const readPrefs = () => {
  try {
    return fs.existsSync(prefsPath()) ? JSON.parse(fs.readFileSync(prefsPath(), 'utf8')) : { mode: 'client', masterPort: 3001, apiUrl: 'http://127.0.0.1:3001/api' };
  } catch { return { mode: 'client', masterPort: 3001, apiUrl: 'http://127.0.0.1:3001/api' }; }
};
...
const createWindow = () => {
  mainWindow = new BrowserWindow({ width: 1280, height: 850, minWidth: 960, minHeight: 640, webPreferences: { nodeIntegration: false, contextIsolation: true, preload: path.join(__dirname, 'preload.cjs') }, icon: path.join(__dirname, 'icon.ico') });
  if (isDev) { mainWindow.loadURL('http://localhost:5173'); mainWindow.webContents.openDevTools(); }
  else mainWindow.loadFile(path.join(__dirname, '../dist/index.html'));
};
const applyNetworkMode = async (prefs) => {
  if (prefs.mode === 'master') return centralServer.start(prefs.masterPort || 3001, prefs.backupPath || null, { force: Boolean(prefs.forceMaster) });
...
app.whenReady().then(async () => {
  ...
  try { await applyNetworkMode(prefs); }
  catch (error) {
    console.error('Avvio automatico server centrale fallito:', error.message);
    savePrefs({ ...prefs, mode: 'client', lastServerError: error.message });
  }
  createWindow();
});
...
ipcMain.handle('network-save-prefs', async (event, incoming) => {
  try {
    ...
    return { success: true, prefs, server: result };
  } catch (error) { return { success: false, error: error.message, code: error.code, masters: error.masters }; }
});
ipcMain.handle('server-start', async (event, port, backupPath, force = false) => {
  try { return await centralServer.start(port, backupPath, { force }); }
  catch (error) { return { success: false, error: error.message, code: error.code, masters: error.masters }; }
});
...
ipcMain.handle('network-test-api', async (event, apiUrl) => {
  ...
  try {
    const response = await fetch(`${String(apiUrl).replace(/\/$/, '')}/health`, { signal: controller.signal, cache: 'no-store' });
    return { success: response.ok, data: await response.json() };
  } catch (error) { return { success: false, error: error.message }; }
  finally { clearTimeout(timeout); }
});
ipcMain.handle('sync-with-master', async () => ({ success: false, error: 'La sincronizzazione a copie è stata sostituita dal server centrale.' }));
ipcMain.handle('push-to-master', async () => ({ success: false, error: 'Le modifiche devono passare attraverso le API centrali.' }));
